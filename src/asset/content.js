const content = {
  uz: {
    madeBy: "Yaratuvchi: Abduhalim!",
    emailLabelStart: "Email manzil:",
    emailPlaceholder: "Emailingizni kiriting...",
    emailLabelEnd: "Emailingiz tarqalmaydi!",
    parolLabelStart: "Parolingiz:",
    parolPlaceholder: "Parolingizni kiriting...",
    parolLabelEnd: "Parolingiz tarqalmaydi!",
    remember: "Eslab qolish!",
    send: "Yuborish!",
  },
  en: {
    madeBy: "Creator: Abdukhalim!",
    emailLabelStart: "Email:",
    emailPlaceholder: "Input your email:",
    emailLabelEnd: "we won`t send your email to anyone!",
    parolLabelStart: "Password:",
    parolPlaceholder: "Input your password...",
    parolLabelEnd: "we won`t send your password to anyone!",
    remember: "Keep sign in!",
    send: "Send!",
  },
  ru: {
    madeBy: "Создатель: Абдухалим!",
    emailLabelStart: "Эл. почта:",
    emailPlaceholder: "Введите свой адрес электронной почты...",
    emailLabelEnd: "ваша электронная почта не будет распространяться!",
    parolLabelStart: "Пароль:",
    parolPlaceholder: "Введите пароль...",
    parolLabelEnd: "Ваш пароль не будет выдан!",
    remember: "Вспомнить!",
    send: "Отправить!",
  },
};

export default content;
